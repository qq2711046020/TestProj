...
//BEGIN:CUSTOM
extern "C" {
    extern "C" int luaopen_checks(lua_State*);

    extern "C" int luaopen_pb(lua_State*);
    extern "C" int luaopen_pb_buffer(lua_State*);
    extern "C" int luaopen_pb_conv(lua_State*);
    extern "C" int luaopen_pb_io(lua_State*);
    extern "C" int luaopen_pb_slice(lua_State*);
    extern "C" int luaopen_pb_unsafe(lua_State*);
}
//END
...
void FLuaContext::CreateState()
{
    ...
    if (!L)
    {
        ...
        lua_register(L, "UEWarn", Global_Warn);
...
#if WITH_EDITOR//fixed
					if (!ModuleName.Contains(Class->GetName()))//fixed
					{//fixed
						UE_LOG(LogUnLua, Error, TEXT("ModuleName(%s) should match class name (%s)"), *ModuleName, *Class->GetName());//fixed
					}//fixed
#endif//fixed
                    Class = OuterClass == InterfaceClass ? Class : OuterClass;      // select the target UClass to bind Lua module
                    /*if (ModuleName.Len() < 1)//FIXED 
                    {
...
bool FLuaContext::CheckBindable(UObject *InObject)
{
	static UClass* InterfaceClass = UUnLuaInterface::StaticClass();
	...
	{
#if WITH_EDITOR
		if (GIsEditor && InObject->GetOuter())
		{
			UWorld* World = InObject->GetOuter()->GetWorld();
			if (World && !World->IsGameWorld())
			{
				return false;
			}
...
void TryToBindGameInstance(UWorld* World)
{
    if (!FLuaContext::Create()->IsEnable()) return;

	if (UGameEngine*Engine = Cast<UGameEngine>(GEngine))
	{
        if (Engine->GameInstance)
        {
            FLuaContext::Create()->TryToBindLua(Engine->GameInstance);                     // try to bind Lua module for GameInstance
        }
	}
}
...
#else // 打包之后，lua实例化太晚了// FIXED
    if (bEnable == false)
    {
        SetEnable(true);// FIXED
    }
#endif

    TryToBindGameInstance(World);// FIXED
}
...
void FLuaContext::PostLoadMapWithWorld(UWorld *World)
{
    ...
    // GameInstance 绑定时间太晚，移动到OnPostWorldInitialization
...
void FLuaContext::Initialize()
{
    ...
    if (L)
    {
        ...
        UnLua::Call(L, "require", "Init");//CUSTOM
...
void FLuaContext::Cleanup(bool bFullCleanup, UWorld *World)
{
    ...
    if (L)
    {
        ...
        ModuleNames.Empty();
		GEngine->ForceGarbageCollection();//fixed
        if (bFullCleanup)
        {
            LuaUnrealNet.Reset();//CUSTOM
