...
bool UUnLuaManager::Bind(UObjectBaseUtility* Object, UClass* Class, const TCHAR* InModuleName, int32 InitializerTableRef)
{
	...
	if (FCString::Strcmp(InModuleName, TEXT("")) == 0)// FIXED
	{
		return false;// FIXED
	}// FIXED

	if (Object->GetOuter()->GetName().Contains(TEXT("WidgetTree")) && Object->HasAnyFlags(RF_NeedPostLoad | RF_NeedPostLoadSubobjects | RF_WasLoaded))
	{
		return false;
	}
...
//		UE_LOG(LogUnLua, Log, TEXT("%s Bind %s and  call 'Initialize' function!"), *Object->GetName(), InModuleName);
//#endif
	}
	else
	{
...
void UUnLuaManager::NotifyUObjectDeleted(const UObjectBase* Object, bool bClass)
{
	...
	/*UClass* c = Object->GetClass();

	UE_LOG(LogUnLua, Warning, TEXT("NotifyUObjectDeleted Class is %s") , *c->GetFName().ToString());*/
...
//// Fixed Gameinstance Create ShouldNotCleanup
bool UUnLuaManager::IsGlobalObject(const UObject* inObj) const
{
	const UObject* outer = inObj;
	while (outer && !inObj->GetClass()->IsChildOf(UUserWidget::StaticClass()))
	{
		if (outer == GetTransientPackage() || outer->GetClass()->IsChildOf(UGameInstance::StaticClass()))
		{
			return true;
		}
...
void UUnLuaManager::Cleanup(UWorld* InWorld, bool bFullCleanup)
{
	if (AttachedObjects.Num() > 0)
	{
		for (TMap<UObjectBaseUtility*, int32>::TIterator It(AttachedObjects); It; ++It)
		{
			...
			//UE_LOG(LogUnLua, Warning, TEXT("CLEANUP Step 1/2(%d): %s"), AttachedObjects.Num(), *Object->GetName());
			if (Object && InWorld)
			{
				UWorld* World = Object->GetWorld();
				if (World && World != InWorld || IsGlobalObject(Object))//FIX GameInstance析构
				{
					continue;
				}
			}
			...
			// 这里需要解除引用，否则阻止UMG等的GC
			GObjectReferencer.RemoveObjectRef(Object);//Fixed
			//UE_LOG(LogUnLua, Warning, TEXT("CLEANUP Step 2/2(%d): %s"), AttachedObjects.Num(), *Object->GetName());
		}
		if (bFullCleanup)
		{
