        ...
        : FPropertyDesc(InProperty), MetaClass(nullptr)
    {
        if (ObjectBaseProperty->PropertyClass->IsChildOf(UClass::StaticClass()))
        {
            MetaClass = bSoftObject ? (((FSoftClassProperty*)Property)->MetaClass) : ((FClassProperty*)Property)->MetaClass;
 			RegisterClass(*GLuaCxt, MetaClass);//FIXED
        }
...
//FIXED////////////////////////////////////////////////////////////////
/**
 * LazyObject property descriptor
 */
class FLazyObjectPropertyDesc : public FStructPropertyDesc
{
public:
    explicit FLazyObjectPropertyDesc(FProperty* InProperty)
        : FStructPropertyDesc(InProperty)
    {
        RegisterClass(*GLuaCxt, LazyObjectProperty->PropertyClass);
    }
    ...
    virtual void GetValueInternal(lua_State* L, const void* ValuePtr, bool bCreateCopy) const override
    {
        if (bCreateCopy)
        {
            void* Userdata = NewUserdataWithPadding(L, Property->GetSize(), "FLazyObjectPtr");
            Property->InitializeValue(Userdata);
            Property->CopyCompleteValue(Userdata, ValuePtr);
        }
        else
        {
            if (Property->ArrayDim > 1)
            {
                PushLazyObjectArray(L, LazyObjectProperty, (void*)ValuePtr);
            }
            else
            {
                UnLua::PushPointer(L, (void*)(ValuePtr), "FLazyObjectPtr", bFirstPropOfScriptStruct);
            }
        }
    }
    ...
    virtual bool SetValueInternal(lua_State* L, void* ValuePtr, int32 IndexInStack, bool bCopyValue) const override
    {
        ...
        if (Value)
        {
            if (!bCopyValue && Property->HasAnyPropertyFlags(CPF_OutParm))
            {
                FMemory::Memcpy(ValuePtr, Value, sizeof(FLazyObjectPtr));           // shallow copy
                return false;
            }
            else
            {
...
/**
 * WeakObject property descriptor
 */
class FWeakObjectPropertyDesc : public FStructPropertyDesc
{
public:
    explicit FWeakObjectPropertyDesc(FProperty* InProperty)
        : FStructPropertyDesc(InProperty)
    {
        RegisterClass(*GLuaCxt, WeakObjectProperty->PropertyClass);
    }
    ...
    virtual void GetValueInternal(lua_State* L, const void* ValuePtr, bool bCreateCopy) const override
    {
        if (bCreateCopy)
        {
            void* Userdata = NewUserdataWithPadding(L, Property->GetSize(), "FWeakObjectPtr");
            Property->InitializeValue(Userdata);
            Property->CopyCompleteValue(Userdata, ValuePtr);
        }
        else
        {
            if (Property->ArrayDim > 1)
            {
                PushWeakObjectArray(L, WeakObjectProperty, (void*)ValuePtr);
            }
            else
            {
                UnLua::PushPointer(L, (void*)(ValuePtr), "FWeakObjectPtr", bFirstPropOfScriptStruct);
            }
        }
    }
    ...
    virtual bool SetValueInternal(lua_State* L, void* ValuePtr, int32 IndexInStack, bool bCopyValue) const override
    {
        ...
        if (Value)
        {
            if (!bCopyValue && Property->HasAnyPropertyFlags(CPF_OutParm))
            {
                FMemory::Memcpy(ValuePtr, Value, sizeof(FWeakObjectPtr));           // shallow copy
                return false;
            }
            else
            {
...
FPropertyDesc* FPropertyDesc::Create(FProperty *InProperty)
{
    ...
    case CPT_ObjectReference:
        return new FObjectPropertyDesc(InProperty,false);//FIXED
    case CPT_WeakObjectReference:
        return new FWeakObjectPropertyDesc(InProperty);//FIXED
